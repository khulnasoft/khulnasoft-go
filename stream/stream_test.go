...
func TestStreamNewWithOptionalParams(t *testing.T) {
	...
	err := client.Stream.New(context.TODO(), stream.StreamNewParams{
		...
		TusResumable:   khulnasoft.F(stream.StreamNewParamsTusResumable1_0_0),
		UploadLength:   khulnasoft.F(int64(0)),
...
func TestStreamListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Stream.List(context.TODO(), stream.StreamListParams{
		AccountID:     khulnasoft.F("023e105f4ecef8ad9ca31a8372d0c353"),
		Asc:           khulnasoft.F(true),
		Creator:       khulnasoft.F("creator-id_abcde12345"),
		End:           khulnasoft.F(time.Now()),
		IncludeCounts: khulnasoft.F(true),
		Search:        khulnasoft.F("puppy.mp4"),
		Start:         khulnasoft.F(time.Now()),
		Status:        khulnasoft.F(stream.StreamListParamsStatusPendingupload),
		Type:          khulnasoft.F("live"),
	})
	if err != nil {
		var apierr *khulnasoft.Error
		if errors.As(err, &apierr) {
