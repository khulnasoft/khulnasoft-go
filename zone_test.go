...
func mockZone(i int) *Zone {
	...
	return &Zone{
		...
		DevMode: 0,
		OriginalNS: []string{
			"linda.ns.khulnasoft.com",
			"merlin.ns.khulnasoft.com",
		},
		OriginalRegistrar: "khulnasoft, inc. (id: 1910)",
		OriginalDNSHost:   "",
		CreatedOn:         mustParseTime("2021-07-28T05:06:20.736244Z"),
		ModifiedOn:        mustParseTime("2021-07-28T05:06:20.736244Z"),
		NameServers: []string{
			"abby.ns.khulnasoft.com",
			"noel.ns.khulnasoft.com",
		},
		Owner: Owner{
			ID:        mockID(ownerName),
			Email:     "",
...
func TestZoneAnalyticsDashboard(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		// JSON data from: https://api.khulnasoft.com/#zone-analytics-properties
		fmt.Fprintf(w, `{
          "success": true,
          "errors": [],
          "messages": [],
...
func TestZoneAnalyticsByColocation(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("content-type", "application/json")
		// JSON data from: https://api.khulnasoft.com/#zone-analytics-analytics-by-co-locations
		fmt.Fprintf(w, `{
          "success": true,
          "errors": [],
          "messages": [],
...
var expectedFullZoneSetup = Zone{
	...
	Paused:      false,
	Type:        "full",
	NameServers: []string{"tony.ns.khulnasoft.com", "woz.ns.khulnasoft.com"},
}
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{
			...
			"result": {
				...
				"type": "full",
				"name_servers": [
					"tony.ns.khulnasoft.com",
					"woz.ns.khulnasoft.com"
				]
			}
		}
		`)
...
func TestZonePartialHasVerificationKey(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("content-type", "application/json")
		// JSON data from: https://api.khulnasoft.com/#zone-zone-details (plus an undocumented field verification_key from curl to API)
		fmt.Fprintf(w, `{
  "result": {
    "id": "foo",
    "name": "bar",
...
func TestZoneDNSSECSetting(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("content-type", "application/json")
		// JSON data from: https://api.khulnasoft.com/#dnssec-properties
		fmt.Fprintf(w, `{
			"result": {
				"status": "active",
				"flags": 257,
...
func TestUpdateZoneSSLSettings(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodPatch, r.Method, "Expected method 'PATCH', got %s", r.Method)
		w.Header().Set("content-type", "application/json")
		// JSON data from: https://api.khulnasoft.com/#zone-settings-properties
		_, _ = fmt.Fprintf(w, `{
			"result": {
				"id": "ssl",
				"value": "off",
