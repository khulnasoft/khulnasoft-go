...
func TestLockdownNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Firewall.Lockdowns.New(
		...
		firewall.LockdownNewParams{
			Configurations: khulnasoft.F[firewall.ConfigurationUnionParam](firewall.LockdownIPConfigurationParam{
				Target: khulnasoft.F(firewall.LockdownIPConfigurationTargetIP),
...
func TestLockdownListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Firewall.Lockdowns.List(
		...
		firewall.LockdownListParams{
			CreatedOn:         khulnasoft.F(time.Now()),
			Description:       khulnasoft.F("endpoints"),
			DescriptionSearch: khulnasoft.F("endpoints"),
			IP:                khulnasoft.F("1.2.3.4"),
			IPRangeSearch:     khulnasoft.F("1.2.3.0/16"),
			IPSearch:          khulnasoft.F("1.2.3.4"),
			ModifiedOn:        khulnasoft.F(time.Now()),
			Page:              khulnasoft.F(1.000000),
			PerPage:           khulnasoft.F(1.000000),
			Priority:          khulnasoft.F(5.000000),
			URISearch:         khulnasoft.F("/some/path"),
		},
	)
	if err != nil {
		var apierr *khulnasoft.Error
