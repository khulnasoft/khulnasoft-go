	...
	singleStreamResponse = `
{
  ...
  "result": {
    ...
    "preview": "https://watch.khulnasoftstream.com/ea95132c15732412d22c1476fa83f27a",
    "readyToStream": true,
...
func TestStream_EmbedHTML(t *testing.T) {
	...
	streamHTML := `<stream id="ea95132c15732412d22c1476fa83f27a"></stream><script data-cfasync="false" defer type="text/javascript" src="https://embed.khulnasoftstream.com/embed/we4g.fla9.latest.js"></script>`
...
func TestStream_StreamInitiateTUSVideoUpload(t *testing.T) {
	...
	mux.HandleFunc("/accounts/"+testAccountID+"/stream", func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("direct_user") == "true" {
			...
			w.Header().Set("Location", "https://api.khulnasoft.com/client/v4/accounts/"+testAccountID+"/media/278f2a7e763c73dedc064b965d2cfbed?tusv2=true")
		}
	...
	if assert.NoError(t, err) {
		assert.Equal(t, "https://api.khulnasoft.com/client/v4/accounts/"+testAccountID+"/media/278f2a7e763c73dedc064b965d2cfbed?tusv2=true", out.ResponseHeaders.Get("Location"))
