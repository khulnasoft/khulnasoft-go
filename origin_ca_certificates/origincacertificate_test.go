...
func TestOriginCACertificateNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.OriginCACertificates.New(context.TODO(), origin_ca_certificates.OriginCACertificateNewParams{
		...
		Hostnames:         khulnasoft.F([]string{"example.com", "*.example.com"}),
		RequestType:       khulnasoft.F(shared.CertificateRequestTypeOriginRSA),
		RequestedValidity: khulnasoft.F(ssl.RequestValidity7),
	})
	if err != nil {
