// Package khulnasoft implements the Khulnasoft v4 API.
package khulnasoft
...
// New creates a new Khulnasoft v4 API client.
...
// NewWithAPIToken creates a new Khulnasoft v4 API client using API Tokens.
...
// NewWithUserServiceKey creates a new Khulnasoft v4 API client using service key authentication.
...
func (api *API) makeRequestWithAuthTypeAndHeadersComplete(ctx context.Context, method, uri string, params interface{}, authType int, headers http.Header) (*APIResponse, error) {
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		if resp.StatusCode >= http.StatusInternalServerError {
			return nil, &ServiceError{khulnasoftError: &Error{
		...
		case http.StatusUnauthorized:
			...
			return nil, &AuthorizationError{khulnasoftError: err}
		case http.StatusForbidden:
			err.Type = ErrorTypeAuthentication
			return nil, &AuthenticationError{khulnasoftError: err}
		case http.StatusNotFound:
			err.Type = ErrorTypeNotFound
			return nil, &NotFoundError{khulnasoftError: err}
		case http.StatusTooManyRequests:
			err.Type = ErrorTypeRateLimit
			return nil, &RatelimitError{khulnasoftError: err}
		default:
			err.Type = ErrorTypeRequest
			return nil, &RequestError{khulnasoftError: err}
		}
	}
...
//
// Responses from the actual Khulnasoft servers should pass all these checks (or we
// discover a serious bug in the Khulnasoft servers). However, the unit tests can
