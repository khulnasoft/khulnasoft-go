...
func (ts *testServer) assertMetric(t *testing.T, received []string, expected []string) {
	...
	for idx := 0; idx < min; idx++ {
		if strings.HasPrefix(expected[idx], "khulnasoft.dogstatsd.client.bytes_sent") {
			continue
		}
		if strings.HasPrefix(expected[idx], "khulnasoft.dogstatsd.client.packets_sent") {
			continue
		}
...
func (ts *testServer) getTelemetry() []string {
	...
	metrics := []string{
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics:%d|c%s", totalMetrics, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.events:%d|c%s", ts.telemetry.event, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.service_checks:%d|c%s", ts.telemetry.service_check, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metric_dropped_on_receive:%d|c%s", ts.telemetry.metric_dropped_on_receive, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.packets_sent:%d|c%s", ts.telemetry.packets_sent, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.packets_dropped:%d|c%s", ts.telemetry.packets_dropped, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.packets_dropped_queue:%d|c%s", ts.telemetry.packets_dropped_queue, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.packets_dropped_writer:%d|c%s", ts.telemetry.packets_dropped_writer, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.bytes_sent:%d|c%s", ts.telemetry.bytes_sent, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.bytes_dropped:%d|c%s", ts.telemetry.bytes_dropped, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.bytes_dropped_queue:%d|c%s", ts.telemetry.bytes_dropped_queue, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.bytes_dropped_writer:%d|c%s", ts.telemetry.bytes_dropped_writer, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics_by_type:%d|c%s,metrics_type:gauge", ts.telemetry.gauge, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics_by_type:%d|c%s,metrics_type:count", ts.telemetry.count, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics_by_type:%d|c%s,metrics_type:histogram", ts.telemetry.histogram, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics_by_type:%d|c%s,metrics_type:distribution", ts.telemetry.distribution, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics_by_type:%d|c%s,metrics_type:set", ts.telemetry.set, tags) + containerID,
		fmt.Sprintf("khulnasoft.dogstatsd.client.metrics_by_type:%d|c%s,metrics_type:timing", ts.telemetry.timing, tags) + containerID,
	}

	if ts.aggregation {
		metrics = append(metrics, []string{
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context:%d|c%s", ts.telemetry.aggregated_context, tags) + containerID,
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context_by_type:%d|c%s,metrics_type:gauge", ts.telemetry.aggregated_gauge, tags) + containerID,
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context_by_type:%d|c%s,metrics_type:count", ts.telemetry.aggregated_count, tags) + containerID,
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context_by_type:%d|c%s,metrics_type:set", ts.telemetry.aggregated_set, tags) + containerID,
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context_by_type:%d|c%s,metrics_type:distribution", ts.telemetry.aggregated_distribution, tags) + containerID,
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context_by_type:%d|c%s,metrics_type:histogram", ts.telemetry.aggregated_histogram, tags) + containerID,
			fmt.Sprintf("khulnasoft.dogstatsd.client.aggregated_context_by_type:%d|c%s,metrics_type:timing", ts.telemetry.aggregated_timing, tags) + containerID,
		}...)
	}
	return metrics
}
