...
func TestDiscoveryOperationListWithOptionalParams(t *testing.T) {
	...
	_, err := client.APIGateway.Discovery.Operations.List(context.TODO(), api_gateway.DiscoveryOperationListParams{
		...
		Diff:      khulnasoft.F(true),
		Direction: khulnasoft.F(api_gateway.DiscoveryOperationListParamsDirectionAsc),
		Endpoint:  khulnasoft.F("/api/v1"),
		Host:      khulnasoft.F([]string{"api.khulnasoft.com"}),
		Method:    khulnasoft.F([]string{"GET"}),
		Order:     khulnasoft.F(api_gateway.DiscoveryOperationListParamsOrderHost),
		Origin:    khulnasoft.F(api_gateway.DiscoveryOperationListParamsOriginMl),
		Page:      khulnasoft.F(int64(1)),
		PerPage:   khulnasoft.F(int64(5)),
		State:     khulnasoft.F(api_gateway.DiscoveryOperationListParamsStateReview),
	})
	if err != nil {
...
func TestDiscoveryOperationEditWithOptionalParams(t *testing.T) {
	...
	_, err := client.APIGateway.Discovery.Operations.Edit(
		...
		api_gateway.DiscoveryOperationEditParams{
			ZoneID: khulnasoft.F("023e105f4ecef8ad9ca31a8372d0c353"),
			State:  khulnasoft.F(api_gateway.DiscoveryOperationEditParamsStateReview),
		},
	)
	if err != nil {
		var apierr *khulnasoft.Error
