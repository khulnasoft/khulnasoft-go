...
func TestDatasetListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.Datasets.List(context.TODO(), radar.DatasetListParams{
		DatasetType: khulnasoft.F(radar.DatasetListParamsDatasetTypeRankingBucket),
		Format:      khulnasoft.F(radar.DatasetListParamsFormatJson),
		Limit:       khulnasoft.F(int64(5)),
		Offset:      khulnasoft.F(int64(0)),
	})
	if err != nil {
...
func TestDatasetDownloadWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.Datasets.Download(context.TODO(), radar.DatasetDownloadParams{
		DatasetID: khulnasoft.F(int64(3)),
		Format:    khulnasoft.F(radar.DatasetDownloadParamsFormatJson),
	})
	if err != nil {
		var apierr *khulnasoft.Error
		if errors.As(err, &apierr) {
