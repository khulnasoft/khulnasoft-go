...
func TestQualityIQISummaryWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.Quality.IQI.Summary(context.TODO(), radar.QualityIQISummaryParams{
		Metric:    khulnasoft.F(radar.QualityIQISummaryParamsMetricBandwidth),
		...
		Format:    khulnasoft.F(radar.QualityIQISummaryParamsFormatJson),
...
func TestQualityIQITimeseriesGroupsWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.Quality.IQI.TimeseriesGroups(context.TODO(), radar.QualityIQITimeseriesGroupsParams{
		Metric:        khulnasoft.F(radar.QualityIQITimeseriesGroupsParamsMetricBandwidth),
		AggInterval:   khulnasoft.F(radar.QualityIQITimeseriesGroupsParamsAggInterval15m),
		ASN:           khulnasoft.F([]string{"string", "string", "string"}),
		Continent:     khulnasoft.F([]string{"string", "string", "string"}),
		DateEnd:       khulnasoft.F([]time.Time{time.Now(), time.Now(), time.Now()}),
		DateRange:     khulnasoft.F([]string{"7d", "7d", "7d"}),
		DateStart:     khulnasoft.F([]time.Time{time.Now(), time.Now(), time.Now()}),
		Format:        khulnasoft.F(radar.QualityIQITimeseriesGroupsParamsFormatJson),
		Interpolation: khulnasoft.F(true),
		Location:      khulnasoft.F([]string{"string", "string", "string"}),
		Name:          khulnasoft.F([]string{"string", "string", "string"}),
	})
	if err != nil {
