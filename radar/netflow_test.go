...
func TestNetflowSummaryWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.Netflows.Summary(context.TODO(), radar.NetflowSummaryParams{
		...
		Format:    khulnasoft.F(radar.NetflowSummaryParamsFormatJson),
...
func TestNetflowTimeseriesWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.Netflows.Timeseries(context.TODO(), radar.NetflowTimeseriesParams{
		AggInterval:   khulnasoft.F(radar.NetflowTimeseriesParamsAggInterval15m),
		ASN:           khulnasoft.F([]string{"string", "string", "string"}),
		Continent:     khulnasoft.F([]string{"string", "string", "string"}),
		DateEnd:       khulnasoft.F([]time.Time{time.Now(), time.Now(), time.Now()}),
		DateRange:     khulnasoft.F([]string{"7d", "7d", "7d"}),
		DateStart:     khulnasoft.F([]time.Time{time.Now(), time.Now(), time.Now()}),
		Format:        khulnasoft.F(radar.NetflowTimeseriesParamsFormatJson),
		Location:      khulnasoft.F([]string{"string", "string", "string"}),
		Name:          khulnasoft.F([]string{"string", "string", "string"}),
		Normalization: khulnasoft.F(radar.NetflowTimeseriesParamsNormalizationPercentageChange),
		Product:       khulnasoft.F([]radar.NetflowTimeseriesParamsProduct{radar.NetflowTimeseriesParamsProductHTTP, radar.NetflowTimeseriesParamsProductAll}),
	})
	if err != nil {
		var apierr *khulnasoft.Error
		if errors.As(err, &apierr) {
