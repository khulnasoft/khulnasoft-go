	...
	}{
		{
			...
			params: ListAPIShieldOperationsParams{
				...
				APIShieldListOperationsFilters: APIShieldListOperationsFilters{
					Hosts:    []string{"api.khulnasoft.com", "developers.khulnasoft.com"},
			...
			expectedParams: url.Values{
				...
				"order":     []string{"host"},
				"host":      []string{"api.khulnasoft.com", "developers.khulnasoft.com"},
...
func TestCreateAPIShieldOperations(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		require.Equal(t, []byte(`[{"method":"POST","host":"api.khulnasoft.com","endpoint":"/client/v4/zones"}]`), body)
