...
func TestDeviceSettingUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.ZeroTrust.Devices.Settings.Update(context.TODO(), zero_trust.DeviceSettingUpdateParams{
		...
		DeviceSettings: zero_trust.DeviceSettingsParam{
			DisableForTime:                     khulnasoft.F(0.000000),
			GatewayProxyEnabled:                khulnasoft.F(true),
			GatewayUdpProxyEnabled:             khulnasoft.F(true),
			RootCertificateInstallationEnabled: khulnasoft.F(true),
			UseZtVirtualIP:                     khulnasoft.F(true),
		},
	})
	if err != nil {
