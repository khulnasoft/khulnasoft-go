...
func TestRiskScoringBehaviourUpdate(t *testing.T) {
	...
	_, err := client.ZeroTrust.RiskScoring.Behaviours.Update(context.TODO(), zero_trust.RiskScoringBehaviourUpdateParams{
		...
		Behaviors: khulnasoft.F(map[string]zero_trust.RiskScoringBehaviourUpdateParamsBehaviors{
			"foo": {
				Enabled:   khulnasoft.F(true),
				RiskLevel: khulnasoft.F(zero_trust.RiskScoringBehaviourUpdateParamsBehaviorsRiskLevelLow),
			},
		}),
	})
	if err != nil {
