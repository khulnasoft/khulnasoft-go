...
type TunnelConfigurationUpdateResponseConfig struct {
	...
	// between khulnasoftd and origin server.
...
type TunnelConfigurationUpdateResponseConfigIngressOriginRequest struct {
	// For all L7 requests to this hostname, khulnasoftd will validate each request's
	...
	// Hostname that khulnasoftd should expect from your origin server certificate.
	OriginServerName string `json:"originServerName"`
	// khulnasoftd starts a proxy server to translate HTTP traffic into TCP when
