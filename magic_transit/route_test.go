...
func TestRouteUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.MagicTransit.Routes.Update(
		...
		magic_transit.RouteUpdateParams{
			AccountID:   khulnasoft.F("023e105f4ecef8ad9ca31a8372d0c353"),
			Nexthop:     khulnasoft.F("203.0.113.1"),
			Prefix:      khulnasoft.F("192.0.2.0/24"),
			Priority:    khulnasoft.F(int64(0)),
			Description: khulnasoft.F("New route for new prefix 203.0.113.1"),
			Scope: khulnasoft.F(magic_transit.ScopeParam{
				ColoNames:   khulnasoft.F([]string{"den01", "den01", "den01"}),
				ColoRegions: khulnasoft.F([]string{"APAC", "APAC", "APAC"}),
			}),
			Weight: khulnasoft.F(int64(0)),
		},
	)
	if err != nil {
		var apierr *khulnasoft.Error
