...
func formatAccessRule(rule khulnasoft.AccessRule) []string {
	return []string{
		rule.ID,
...
func firewallAccessRules(c *cli.Context) error {
	...
	var response *khulnasoft.AccessRuleListResponse
	switch {
	case accountID != "":
	...
	rules := make([]khulnasoft.AccessRule, 0, response.ResultInfo.Total)
...
func getScope(c *cli.Context) (string, string, error) {
	...
	if c.String("account") != "" {
		...
		params := khulnasoft.AccountsListParams{}
...
func getConfiguration(c *cli.Context) khulnasoft.AccessRuleConfiguration {
	configuration := khulnasoft.AccessRuleConfiguration{}
	if c.String("value") != "" {
