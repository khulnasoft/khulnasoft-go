...
func zoneCreate(c *cli.Context) error {
	...
	var account khulnasoft.Account
	if accountID != "" {
...
func zoneCreateLockdown(c *cli.Context) error {
	...
	var zonelockdownconfigs = []khulnasoft.ZoneLockdownConfig{}
	for index := 0; index < len(targets); index++ {
		zonelockdownconfigs = append(zonelockdownconfigs, khulnasoft.ZoneLockdownConfig{
			Target: c.StringSlice("targets")[index],
			Value:  c.StringSlice("values")[index],
		})
	}
	params := khulnasoft.ZoneLockdownCreateParams{
		Description:    c.String("description"),
		URLs:           c.StringSlice("urls"),
		Configurations: zonelockdownconfigs,
	}

	resp, err := api.CreateZoneLockdown(context.Background(), khulnasoft.ZoneIdentifier(zoneID), params)
	if err != nil {
...
func zoneCachePurge(c *cli.Context) error {
	...
	var resp khulnasoft.PurgeCacheResponse
	...
	if c.Bool("everything") {
		...
		// Purge selectively
		purgeReq := khulnasoft.PurgeCacheRequest{
			Files:    c.StringSlice("files"),
			Tags:     c.StringSlice("tags"),
			Hosts:    c.StringSlice("hosts"),
			Prefixes: c.StringSlice("prefixes"),
...
func zoneRecords(c *cli.Context) error {
	...
	rr := khulnasoft.ListDNSRecordsParams{}
	var records []khulnasoft.DNSRecord
	if c.String("id") != "" {
		rec, err := api.GetDNSRecord(context.Background(), khulnasoft.ZoneIdentifier(zoneID), c.String("id"))
		if err != nil {
			fmt.Println(err)
			return err
		}
		...
		var err error
		records, _, err = api.ListDNSRecords(context.Background(), khulnasoft.ZoneIdentifier(zoneID), rr)
		if err != nil {
			fmt.Println(err)
			return err
		}
	...
	for _, r := range records {
		...
		case "SRV":
			...
			// Khulnasoft's API, annoyingly, automatically prepends the weight
...
func formatCacheResponse(resp khulnasoft.PurgeCacheResponse) []string {
	return []string{
		resp.Result.ID,
	}
}
