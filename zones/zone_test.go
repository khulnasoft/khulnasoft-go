...
func TestZoneListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Zones.List(context.TODO(), zones.ZoneListParams{
		Account: khulnasoft.F(zones.ZoneListParamsAccount{
			ID:   khulnasoft.F("id"),
			Name: khulnasoft.F("name"),
		}),
		Direction: khulnasoft.F(zones.ZoneListParamsDirectionAsc),
		Match:     khulnasoft.F(zones.ZoneListParamsMatchAny),
		Name:      khulnasoft.F("name"),
		Order:     khulnasoft.F(zones.ZoneListParamsOrderName),
		Page:      khulnasoft.F(1.000000),
		PerPage:   khulnasoft.F(5.000000),
		Status:    khulnasoft.F(zones.ZoneListParamsStatusInitializing),
	})
	if err != nil {
		var apierr *khulnasoft.Error
		if errors.As(err, &apierr) {
...
func TestZoneEditWithOptionalParams(t *testing.T) {
	...
	_, err := client.Zones.Edit(context.TODO(), zones.ZoneEditParams{
		ZoneID:            khulnasoft.F("023e105f4ecef8ad9ca31a8372d0c353"),
		Type:              khulnasoft.F(zones.ZoneEditParamsTypeFull),
		VanityNameServers: khulnasoft.F([]string{"ns1.example.com", "ns2.example.com"}),
	})
	if err != nil {
		var apierr *khulnasoft.Error
		if errors.As(err, &apierr) {
