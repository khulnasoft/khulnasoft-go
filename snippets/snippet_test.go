...
func TestSnippetUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Snippets.Update(
		...
		snippets.SnippetUpdateParams{
			...
			Files:  khulnasoft.F("export { async function fetch(request, env) {return new Response('some_response') } }"),
			Metadata: khulnasoft.F(snippets.SnippetUpdateParamsMetadata{
				MainModule: khulnasoft.F("main.js"),
			}),
		},
	)
	if err != nil {
