...
func TestScriptContentUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Workers.Scripts.Content.Update(
		...
		workers.ScriptContentUpdateParams{
			...
			Metadata: khulnasoft.F(workers.WorkerMetadataParam{
				BodyPart:   khulnasoft.F("worker.js"),
				MainModule: khulnasoft.F("worker.js"),
			}),
			CfWorkerBodyPart:       khulnasoft.F("CF-WORKER-BODY-PART"),
			CfWorkerMainModulePart: khulnasoft.F("CF-WORKER-MAIN-MODULE-PART"),
		},
	)
	if err != nil {
